...
def _weather_windows(panel: pd.DataFrame) -> dict:
    ...
    for _, g in panel.groupby("anon_polygon_id", sort=False):
        ...
        def mean_t(lo_d, hi_d, epochs, temperature_epochs, cumulative_temperature):
            a = np.searchsorted(temperature_epochs, epochs + lo_d, "left")
            b = np.searchsorted(temperature_epochs, epochs + hi_d, "right")
            cnt = b - a
            return np.where(
                cnt > 0,
                (cumulative_temperature[b] - cumulative_temperature[a]) / np.maximum(cnt, 1),
                np.nan,
            )

        def sum_p(lo_d, hi_d, epochs, precipitation_epochs, cumulative_precipitation):
            a = np.searchsorted(precipitation_epochs, epochs + lo_d, "left")
            b = np.searchsorted(precipitation_epochs, epochs + hi_d, "right")
            return np.where(
                b > a,
                cumulative_precipitation[b] - cumulative_precipitation[a],
                np.nan,
            )

        out["temp_prev10"][idx] = mean_t(-10, -1, e, et, ct)
        out["temp_prev30"][idx] = mean_t(-30, -1, e, et, ct)
        out["temp_around7"][idx] = mean_t(-7, 7, e, et, ct)
        out["precip_prev10"][idx] = sum_p(-10, -1, e, ep_, cp)
        out["precip_prev30"][idx] = sum_p(-30, -1, e, ep_, cp)
    return out
