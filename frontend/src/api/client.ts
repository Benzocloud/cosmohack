...
export function normalizeError(status: number, body: unknown): AppError {
  ...
  if (!bodyObject) {
    ...
    const nested =
      typeof bodyObject.error === 'object' && bodyObject.error !== null
        ? (bodyObject.error as Record<string, unknown>)
        : null;
    code = asString(nested?.code) ?? 'error';
    title =
      asString(nested?.message) ??
      (typeof bodyObject.error === 'string' ? bodyObject.error : 'request failed');
    detail = asString(nested?.message) ?? asString(bodyObject.message);
  } else {
