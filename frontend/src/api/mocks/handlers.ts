...
const AREAS = (areasFixture.areas as Array<Record<string, unknown>>).map((item) => {
  const legacyResult = item.last_result as Record<string, unknown> | null | undefined;
  const legacySource = item.source as Record<string, unknown> | undefined;
  const legacyJob = item.active_job as Record<string, unknown> | null | undefined;
  return {
    id: item.id,
    name: item.name,
    ...
    source: {
      ...
      contour_id: legacySource?.external_id,
      provider: legacySource?.kind === 'contour' ? 'overpass' : undefined,
    },
    period: legacyResult?.period ?? { from: '2024-03-01', to: '2024-09-30' },
    created_at: item.created_at,
    shown_result: legacyResult
      ? {
          result_version: legacyResult.result_version,
          job_id: legacyJob?.id,
          period: legacyResult.period,
          computed_at: legacyResult.computed_at,
          status: legacyResult.verdict,
          severity: legacyResult.severity,
        }
      : null,
    active_job: legacyJob
      ? { job_id: legacyJob.id, status: legacyJob.status, stage: legacyJob.stage }
      : null,
  } as AreaRaw;
});
...
function completeLiveJob(job: LiveJob, resultVersion: string): void {
  ...
  area.shown_result = {
    ...(area.shown_result ?? { status: 'candidate', severity: null }),
...
export const handlers: HttpHandler[] = [
  ...
  http.get('*/api/regions/contours', ({ request }) => {
    ...
    const fixture = CONTOURS[scenario] as {
      features?: Array<{ geometry?: GeoJSON.Polygon; properties?: { id?: string; name?: string } }>;
    };
    return HttpResponse.json({
      _synthetic: true,
      contours: (fixture.features ?? []).map((feature) => ({
        id: feature.properties?.id,
        name: feature.properties?.name,
        geometry: feature.geometry,
        source: { provider: 'overpass', attribution: 'OpenStreetMap contributors' },
      })),
    });
  }),
  ...
  http.get('*/api/areas/:id/series', ({ request, params }) => {
    ...
    return HttpResponse.json({
      ...
      series: (base.points ?? []).map((point) => {
        ...
        return {
          date: legacy.date,
          primary_ndvi: legacy.ndvi,
          value: legacy.ndvi,
          state: legacy.provenance ?? (legacy.ndvi == null ? 'missing' : 'observed'),
          method: null,
          baseline: null,
          z_score: null,
          interval: null,
      ...
      weather: base.weather === null ? null : [],
      limitations: [],
    });
  }),
  ...
  http.get('*/api/areas/:id/events', ({ request, params }) => {
    ...
    return HttpResponse.json({
      ...
      events: (base.events as Array<Record<string, unknown>>).map((event) => {
        ...
        const detected = event.detected as { magnitude?: number; unit?: string } | undefined;
        return {
          start_date: period?.from,
          end_date: period?.to,
          status: event.verdict,
          severity: event.severity,
          min_z_score: detected?.unit === 'ndvi' ? null : detected?.magnitude,
          evidence_dates: [],
          facts: (
            (event.weather as { facts?: Array<{ value?: string }> } | undefined)?.facts ?? []
          ).map((fact) => fact.value ?? ''),
          hypothesis: (event.weather as { hypothesis?: string } | undefined)?.hypothesis ?? null,
