...
async function fetchSeries(areaId: string, version?: string): Promise<Series> {
  // Ответ — конверт {_synthetic, series}: адаптеру нужен только объект ряда
  const envelope = await apiGet<{ series: SeriesRaw }>('/api/areas/{id}/series', {
    path: { id: areaId },
    query: { version },
  });
  return adaptSeries(envelope.series);
}
...
export function useJob(jobId: string | null | undefined) {
  ...
  const query = useQuery({
    ...
    refetchInterval: (q) => {
      ...
      return job && (job.status === 'queued' || job.status === 'running') ? JOB_POLL_MS : false;
    },
