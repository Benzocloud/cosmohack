...
**Статус:** planned, P1. **Не блокирует MLI-04.**

- [ ] До кода обновить канонический контракт: `schema_version=1.1`, профиль
  `ndvi-multisensor-v1`, `indices`, `area_context`, `peers`, лимиты и ошибки.
- [ ] B1 проверить, какие S2/Landsat/MODIS NDVI/EVI/NDWI реально доступны для
  одного полигона. Не синтезировать отсутствующие сенсорные значения.
- [ ] Определить честный источник `crop_type` и правила выбора соседей; при их
  отсутствии передавать null/пустой список и отражать ограничение.
- [ ] Расширить Go domain/builder/client после контракта; поднять request limit
  до 8 MiB только для согласованного профиля и сохранить строгую валидацию.
- [ ] Добавить negotiation по `/readyz`: Go выбирает v1.1 только если ML
  объявляет профиль и артефакт готов; иначе остаётся на v1.0.
- [ ] Доказать по ответу `method=gradient_boosting_residual`, что HGB реально
  применился, а не только был загружен.
- [ ] Повторить сквозные и браузерные проверки, отдельно записать качество
  web-профиля и отличие от локальной batch-оценки.

**Готово:** новый полигон получает достаточные реальные признаки, Python
использует обученную модель, Go принимает результат v1.1, а fallback явно
наблюдаем и не выдаётся за HGB.
